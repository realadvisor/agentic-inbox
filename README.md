...
Ready classifier questions for the same conversation are grouped into one Jev request (up to three jobs per delivery). Each question retains its own examples, probability, manual overrides and retry token. Different conversation snapshots and requests exceeding the batching payload budget are sent separately. Per-queue concurrency is unchanged; later broker deliveries for already completed sibling jobs are acknowledged without another provider call.
