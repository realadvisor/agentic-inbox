...
Cloudflare's scheduled handler runs every two minutes, with a small immediate kick on manual runs. Work lives in Neon, not the browser: `conversation_classifications` stores jobs/current results, `classifier_runs` and `classifier_run_items` track fixed batches, and two globally leased worker slots bound provider concurrency across invocations. Each scheduled invocation handles up to 20 jobs; the remaining queue continues on subsequent invocations. Expiring leases recover interrupted work. Transient failures retry three total attempts with backoff, jitter and a shared 429 cooldown. Terminal failures remain visible and **Run on existing → Not yet processed** retries them. A cancelled batch does not cancel independent new-mail work.
