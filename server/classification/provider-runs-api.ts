...
const filters = z.object({
	...
	status: z.enum(["running", "succeeded", "failed", "interrupted"]).optional(),
...
export function providerRunsApi(db: Database, admin: boolean) {
	...
	app.get("/", async (c) => {
		...
		const rows =
			await db`SELECT r.id,r.mailbox_id,r.thread_id,r.subject,r.started_at,to_char(r.started_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS cursor_time,r.finished_at,r.duration_ms,r.status,r.http_status,r.requested_model,r.returned_model,r.error,
   ...
   FROM classifier_provider_runs r WHERE true
   ...
   ORDER BY r.started_at DESC,r.id DESC LIMIT ${f.limit + 1}`;
	...
	app.get("/:id", async (c) => {
		...
		const [run] =
			await db`SELECT * FROM classifier_provider_runs WHERE id=${id}`;
		...
		return c.json({ ...run, items, email: email ?? null });
	});
	return app;
}
