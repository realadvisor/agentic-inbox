	...
	try {
		...
		return parts.map((part, i) =>
			part.toLowerCase() === lowerEscaped ? (
				<mark
					key={i}
					className="bg-kumo-warning-muted text-kumo-default rounded-sm px-0.5"
				>
					{part}
				</mark>
			) : (
...
export default function SearchResultsRoute() {
	...
	const { selectedEmailId, isComposing, selectEmail, closePanel } =
		useUIStore();
							...
							{results.map((email) => {
								...
								return (
									<div
										...
										className={`group flex items-center gap-3 w-full text-left cursor-pointer transition-colors border-b border-kumo-line px-4 py-2.5 md:px-5 md:py-3 ${
